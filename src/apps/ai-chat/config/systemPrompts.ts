...
2. **Data-as-Files (WebContainer API)**: NEVER use browser storage (localStorage/IndexedDB). Persist all data to files (e.g., SQLite, JSON) within the app folder.
   ...
   - Instead, if you need a backend, you must scaffold a fullstack app with a separate Node/Express server for \`fs\` operations, OR rely on a mock API/in-memory data if building a pure frontend.
...
5. **SAFE REPLACEMENTS (CRITICAL)**: When using \`replace_in_file\`, strictly prefer exact string matching (\`find\`) over regex to avoid indentation mismatch failures. Target highly unique lines or predefined anchors.
