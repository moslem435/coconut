...
export default function FileExplorer({ initialPath = 'root' }: FileExplorerProps) {
  ...
  const { files, getChildren, getPath, isLoading, loadFolderContent } = useFileSystemStore()
  ...
  // Try to load content if it's a mounted folder
  useEffect(() => {
  ...
  const handleNavigate = async (id: string) => {
    if (files[id]?.type === 'folder') {
      // Check for native mount permission
      const targetPath = useFileSystemStore.getState().resolvePath(id)

      // If it's a mount point or inside one, we should check permission
    ...
    if (item.type === 'file') {
      ...
      if (isImage) {
        launchApp(
          'preview-' + item.id,
          item.name,
          <ImageViewer src={item.content} fileId={item.id} />,
          ImageIcon,
    ...
    <div className="h-full flex flex-col bg-[var(--os-bg-base)] text-[var(--os-text-primary)] pt-10" onContextMenu={handleBackgroundContextMenu}>
      ...
      <div className="h-12 shrink-0 flex items-center gap-2 px-4 border-b border-[var(--os-border)]">
        ...
        <div className="flex items-center gap-2">
          {/* Mount Button */}
          <button
            onClick={() => useFileSystemStore.getState().mountLocalFolder()}
            className="p-1.5 rounded hover:bg-white/10 text-[var(--os-text-secondary)] hover:text-[var(--os-text-primary)] transition-colors"
            title={t('explorer.mount')}
          >
            <HardDrive size={16} />
          </button>
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        {/* Main Content */}
        <div className="flex-1 flex flex-col min-w-0 bg-[var(--os-bg-base)]">
          {/* List Header (Fixed) */}
          {!isLoading && children.length > 0 && viewMode === 'list' && (
            <div className="flex items-center px-4 py-2 text-xs text-[var(--os-text-muted)] border-b border-[var(--os-border)] select-none bg-[var(--os-bg-base)]">
          ...
          <div className="flex-1 overflow-y-auto custom-scrollbar">
            {isLoading ? (
              <div className="h-full flex flex-col items-center justify-center text-[var(--os-text-muted)]">
                <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-[var(--os-text-primary)] mb-4"></div>
