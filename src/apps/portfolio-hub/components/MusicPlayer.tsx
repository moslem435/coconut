...
export default function RadioDeck() {
   ...
   const initAudio = useCallback(() => {
      ...
      audio.src = TRACKS[currentTrack]?.src || TRACKS[0]?.src || ''
      audio.loop = true
   ...
   useEffect(() => {
      if (audioRef.current) {
         ...
         const trackSrc = TRACKS[currentTrack]?.src || TRACKS[0]?.src || ''
   ...
   const drawVisualizer = useCallback(() => {
      ...
      const render = () => {
         ...
         for (let i = 0; i < bufferLength; i++) {
            barHeight = ((dataArray[i] ?? 0) / 255) * canvas.height * 0.9
            ...
            <div className="flex justify-between items-end">
               <div>
                  ...
                  <div className="text-xl font-bold text-orange-400 truncate max-w-[250px]">
                     {TRACKS[currentTrack]?.title || TRACKS[0]?.title || ''}
                  </div>
                  <div className="text-sm text-white/60">{TRACKS[currentTrack]?.artist || TRACKS[0]?.artist || ''}</div>
               </div>
