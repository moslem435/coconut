...
export default function ProjectStage({ activeProjectIndex, selectedSubProject }: ProjectStageProps) {
  ...
  const color = useMemo(() => new THREE.Color(project?.color || '#ffffff'), [project?.color])
        ...
        {isAbout ? (
          ...
          <group key="project-stage" position={[0, 0, 0]} rotation={[0, 0, 0]}>
            ...
            >
              ...
              {`// SYSTEM_ID: ${subProject ? subProject.id : (project?.id || '00')}`}
            </Text>
            <Text
              fontSize={0.2}
              position={[0, -1.3, 0]}
              ...
              fillOpacity={0.5}
            >
              {`// CLASS: ${subProject ? subProject.type : (project?.category || 'UNKNOWN')}`}
            </Text>
          </group>
        )}
      </Center>
